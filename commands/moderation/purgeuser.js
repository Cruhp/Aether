    ...
    run: async (client, message, args) => {
        ...
        if (!user) {
            const mentionEmbed = new MessageEmbed()
                .setColor('#000000')
                .setDescription('<a:Cross:1265733965180960849> **Please mention the user whose messages you want to purge.** ');
        ...
        if (!member) {
            const memberNotFoundEmbed = new MessageEmbed()
                .setColor('#000000')
                .setDescription('<a:Cross:1265733965180960849> **The mentioned user is not found in this server.** ');
        ...
        try {
            ...
            const embed = new MessageEmbed()
                .setColor('#000000')
                .setDescription(`<a:Check:1265733979085078610> **Successfully deleted ${userMessages.size} messages from ${user}.** `);
            message.channel.send({ embeds: [embed] }).then(m => m.delete({ timeout: 5000 }));

        } catch (error) {
            console.error('Error purging messages:', error);
            message.reply('<a:Cross:1265733965180960849> **There was an error purging messages. Please try again later.** ');
        }
    }
};
